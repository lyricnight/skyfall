...
public abstract class ShaderBase implements Globals {

    private int program;
    private Map<String, Integer> uniformsMap;

    public ShaderBase(final String fragmentShader) {
        int vertexShaderID, fragmentShaderID;

        try (final InputStream vertexStream = getClass().getResourceAsStream("/assets/minecraft/textures/shaders/vertex.vert")) {
            vertexShaderID = createShader(IOUtils.toString(Objects.requireNonNull(vertexStream), Charset.defaultCharset()), ARBVertexShader.GL_VERTEX_SHADER_ARB);
        ...
        try (final InputStream fragmentStream = getClass().getResourceAsStream("/assets/minecraft/textures/shaders/" + fragmentShader)) {
            fragmentShaderID = createShader(IOUtils.toString(Objects.requireNonNull(fragmentStream), Charset.defaultCharset()), ARBFragmentShader.GL_FRAGMENT_SHADER_ARB);
