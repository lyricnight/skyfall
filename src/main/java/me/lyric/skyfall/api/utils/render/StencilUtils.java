...
@UtilityClass
public final class StencilUtils implements Globals {
    public static void checkEXT(Framebuffer framebuffer) {
        if (framebuffer != null) {
            if (framebuffer.depthBuffer > -1) {
                invokeEXT(framebuffer);
                framebuffer.depthBuffer = -1;
            }
        }
    }

    public static void invokeEXT(Framebuffer framebuffer) {
    ...
    public static void writeStencil() {
        mc.getFramebuffer().bindFramebuffer(false);
        checkEXT(mc.getFramebuffer());
    ...
    public static void stencil(int ref) {
